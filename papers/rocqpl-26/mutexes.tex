...
Furthermore, it is vital that there is only one $\mathsf{holds}$ predicate per thread; otherwise, a thread can lock the same lock twice with different instances of $\mathsf{holds}$ and obtain $\mathsf{holds}\ \gamma\ t\ 1\ P \ast \mathsf{holds}\ \gamma\ t\ 1\ P$, therefore capable of borrowing 2 copies of $P$. 
